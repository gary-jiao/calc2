...
<input onclick="document.all.WebBrowser.ExecWB(7,1)" type="button" value="打印预览">
      ...
      <el-container>
        ...
        <el-main style="padding:0px;">
              ...
              <table>
                <tbody>
                  <tr>
                    <td>可用运算符：</td>
                    <td>
                      <el-checkbox style="display:inline-block" v-model="isadd" label="加" border />
                      <el-checkbox style="display:inline-block" v-model="issub" label="减" border />
                      <el-checkbox style="display:inline-block" v-model="ismul" label="乘" border />
                      <el-checkbox
                  ...
                  <tr>
                    ...
                    <td>
                      <el-select v-model="rule" placeholder="请选择">
                        <el-option label="已知条件，求得数" value="1"></el-option>
                        <el-option label="已知得数，求条件" value="2"></el-option>
                      </el-select>
                      <el-select v-if="'2'==rule" v-model="whichcond" placeholder="请选择">
                        <el-option label="随机求条件" value></el-option>
                        <el-option
                          v-for="(rg, i) in range"
                          :key="i"
                          :label="'求条件 '+(i+1)"
                  ...
                  <tr>
                    ...
                    <td>
                      <el-tooltip
                        class="item"
                        effect="dark"
                        content="注意借/进位设置只对加/减法起作用"
                        placement="bottom-start"
                      >
                        <el-select v-model="borrow" placeholder="请选择">
                          <el-option label="随机决定" value="random"></el-option>
                          <el-option label="不要借/进位" value="no"></el-option>
                          <el-option label="全都带借/进位" value="all"></el-option>
                        </el-select>
                      </el-tooltip>
                    </td>
                  </tr>
          ...
          <el-alert
            ...
            type="success"
            show-icon
          >总题数：{{res.length}}，其中：加法：{{report.addcnt}}，减法：{{report.subcnt}}，乘法：{{report.mulcnt}}，除法：{{report.divcnt}}，借/进位：{{report.borrowcnt}}，异常：{{report.exceptcnt}}</el-alert>
        </el-main>
      </el-container>
    </div>
        ...
        <tr>
          <th :colspan="cols">
            ...
            <div v-popover:popover1 class="print-repeat" contenteditable="true">
              <h3>100以内口算 日期:____________________ 时间:_______________</h3>
            </div>
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-if="res.length <= 0">
          <td>
            <el-alert title="还没有生成试题！" type="error" :closable="false"></el-alert>
          </td>
        </tr>
        <tr
          :class="{break_before:p>1}"
            ...
            >
              ...
              >
                <td v-for="c in (cols-0)">
                  <template
                    v-if="r > parseInt((res.length - (p - 1) * (pagerows-0) * (cols-0) + (cols - 1)) / (cols-0))"
                  >&nbsp;</template>
                  <template
                    v-if="(p-1) * (pagerows-0) * (cols-0) + (r-1) * (cols-0) + (c - 1) < res.length"
                  >
                    <span
                      v-for="o in res[ (p-1) * (pagerows-0) * (cols-0) + (r-1) * (cols-0) + (c - 1)].li"
                    >{{myfmt(o)}}</span>
                  </template>
                </td>
              </tr>
    ...
    <el-dialog title="请选择要加载的试卷" :visible.sync="paperDialogVisible" width="30%">
      <el-table
        height="250"
        ref="singleTable"
        :data="tableData"
      ...
      >
        ...
        <el-table-column property="date" label="保存时间" width="180"></el-table-column>
        <el-table-column property="name" label="名称" width="180"></el-table-column>
        <el-table-column property="op" label="操作"></el-table-column>
      </el-table>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data: function() {
    return {
      ...
      itemcount: 0,
      defrange: { min: 0, max: 100 }, // 运算项1/2取值范围
      result: { min: 0, max: 100 }, // 得数取值范围
      range: [],
      borrow: "random",
      fontsize: 26,
      fontfamily: "宋体",
  ...
  created: function() {
    Bmob.initialize(
      "cb918bec30eca29246f7101f5ffebafe",
      "bc68d3c114c13ec55fe79fbb7df3f166"
    );
    this.itemcount = 2;
      ...
      for (var i = 0; i < val; i++) {
        if (!this.range[i]) {
          this.range.splice(i, 1, {
            min: this.defrange.min,
            max: this.defrange.max
          });
        }
      }
  ...
  methods: {
    ...
    genItem: function() {
      ...
      var w =
        "" === this.whichcond
          ? randomInt(0, this.itemcount - 1)
          : this.whichcond - 0; // 已知得数，随机求某一个条件
      var min = this.range[0].min,
        max = this.range[0].max,
        limit,
        isexcept = false,
        isborrow = false;
      var op = this.op(),
      ...
      if ("+" == op) {
        ...
        rg1 = {
          min: this.result.min - this.range[1].min,
          max: this.result.max - this.range[1].min
        };
      } else if ("-" == op) {
        ...
        if ("all" == this.borrow) {
          min = Math.max(
            min,
            this.range[1].min - 0 + (this.result.min - 0) + 10
          ); // 强制借位时，必须比减数至少大 10 以上才行!
        } else {
          min = Math.max(min, this.range[1].min - 0 + (this.result.min - 0)); // 被减数不允许比(减数+得数)还小，这会产生负数结果
        }
        rg1 = {
          min: this.result.min - 0 + (this.range[1].min - 0),
          max: this.result.max - 0 + (this.range[1].min - 0)
        };
      } else if ("*" == op) {
        ...
        rg1 = {
          min:
            this.range[1].min == 0
              ? 0
              : Math.round(this.result.min / this.range[1].min),
          max:
            this.range[1].min == 0
              ? this.result.max
              : Math.round(this.result.max / this.range[1].min)
        };
      } else if ("/" == op) {
        // @todo 实现除法
        rg1 = {
      ...
      for (var i = 1; i < this.itemcount; i++, op = this.op()) {
        ...
        var rgc = {
          min: eval(r + op + this.range[i].min),
          max: eval(r + op + this.range[i].max)
        };
        var rgarr = [
          rgc.min - 0,
          rgc.max - 0,
        ...
        if ("*" == op) {
          ...
          // 被乘数为 0? 乘数随机一个值！
          if (r == 0) t = randomInt(this.range[1].min, this.range[1].max);
          // 0 乘以 任何数都等于 0
